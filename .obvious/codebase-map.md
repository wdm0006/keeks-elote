...
| Path | Kind | Purpose |
|---|---|---|
| `keeks_elote/` | Python package | The library. Public surface: `from keeks_elote import Backtest, GameRecord, MatchupTuple, ProjectionRecord, RatingArena, american_to_decimal, calculate_probabilities, create_arena, prepare_data, summarize_bet_history` (pinned by `tests/test_public_api.py`). |
...
| `tests/` | pytest suite | 124 tests: `test_backtest.py`, `test_backtest_settlement.py` (largest — bet settlement/history), `test_arena_factory.py` (create_arena round-trips every supported rating system), `test_data_handling.py`, `test_examples.py`, `test_cfb_example.py`, `test_integration_rating_systems.py` (Massey/Keener/Pythagorean through real elote arenas), `test_model_evaluation.py`, `test_packaging.py`, `test_public_api.py` (pins the package root's surface), `test_rating_arena.py`, `conftest.py`. |
